...
modelpath = './model/{epoch:02d}-{val_loss:.4f}.hdf5'   # 경로의 파일명을 modelpath라고 하겠다. d = 정수, .4 float /소수점 4번째 자리
checkpoint = ModelCheckpoint(filepath = modelpath, monitor='val_loss', save_best_only=True, mode='auto')
# save_best_only 최고로 좋은 값만을 저장하겠다. mode 안써줘도 됨mode는 디폴트값 auto


model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc']) # metrics=['accuracy'] acc, accuracy 통일해야함

hist = model.fit(x_train, y_train, epochs=30, batch_size=128, verbose=1, validation_split=0.2, callbacks = [early_stopping, checkpoint])
